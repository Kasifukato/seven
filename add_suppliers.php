...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['save_supplier'])) {
        ...
        $product = remove_junk($db->escape($_POST['product'])); // Product ID from dropdown
        ...
        if (empty($name) || empty($email) || empty($contact) || empty($product) || empty($date)) {
            ...
            // Check if the supplier name OR the product is already in use
            $query_check = "SELECT * FROM suppliers WHERE name = '{$name}' OR product_id = '{$product}' LIMIT 1";
            $result_check = $db->query($query_check);

            if ($db->num_rows($result_check) > 0) {
                $session->msg("d", "Either the supplier name or the product is already in use.");
            } else {
                $query = "INSERT INTO suppliers (name, email, contact, product_id, joined_date) 
                          VALUES ('{$name}', '{$email}', '{$contact}', '{$product}', '{$date}')";
                if ($db->query($query)) {
            ...
            <div class="row">
              ...
              <div class="col xs-12 sx-6">
                <div class="site-panel">
                  ...
                  <div class="form__action">
                    <input type="submit" class="button primary-tint" name="save_supplier" value="Save" />
                  </div>
                </div>
              </div>
            </div>
          ...
          <div class="row">
            ...
            <div class="col xs-12 sm-3">
              <div class="form__module">
                ...
                <div class="form__set">
                  <input type="tel" id="contact" name="contact" placeholder="+977 9822-" />
                </div>
              </div>
            </div>
            <div class="col xs-12 sm-3">
